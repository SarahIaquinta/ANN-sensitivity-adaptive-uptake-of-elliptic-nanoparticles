...
import uptake.figures.utils as fiu
import uptake.model.system_definition as sysdef
...
def identify_wrapping_phase(particle, mechanics, membrane, wrapping, energy_computation):
    """
    Clears the local minima from the artefacts and identifies the wrapping phase following
    the process introduced in [1]
...
if __name__ == "__main__":
    ...
    particle = sysdef.ParticleGeometry(r_bar=0.609375, particle_perimeter=2 * pi, sampling_points_circle=300)
    # args.gamma_bar_fs
    mechanics = sysdef.MechanicalProperties_Adaptation(
        testcase="test-energy",
        gamma_bar_r=1,
        gamma_bar_fs=0.16,
        gamma_bar_lambda=95,
        gamma_bar_0=2.09375,
    ...
    membrane = sysdef.MembraneGeometry(particle, sampling_points_membrane=300)
    ...
    # wrapping = sysdef.Wrapping(wrapping_list=wrapping_list)
